...
// a merlin32 OMF load file (.s16), finding "injection" segments (load name
// "bigdata") whose body is just a pathname, and replacing each one in place with a
// real OMF data segment containing the contents of the referenced file - making it
// easy to ship data larger than 64K.
//
...
void helpText()
{
	...
	printf("Replaces each 'bigdata' segment (whose body is a pathname) with the\n");
...
int main(int argc, char* argv[])
{
	...
	// Inject every 'bigdata' segment's referenced data file.
	...
	if (bVerbose)
	{
		printf( "arthur32: injected %d bigdata segment(s)\n", injected );
	}
